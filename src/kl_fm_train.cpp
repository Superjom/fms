...
int main(int argc, char *argv[]) 
{
    ...
    if(cmdline.hasParameter(param_learning_rate)) {
        learning_rate = std::atof(cmdline.getValue(param_learning_rate).c_str());
    }
    ...
    if(input_format == "Data") {
        KL_FM<Data> fms(train_path, test_path, dim, batch_size, learning_rate);
    ...
    if(input_format == "IDData") {
        KL_FM<IDData> fms(train_path, test_path, dim, batch_size, learning_rate);
        fms.train(num_threads, num_iters);
        if(!output_model_path.empty()) {
            fms.model_to(output_model_path);
        }
